...
def main(APIToken='',
    ...
    """
    print('WaPOR Interception: Download dekadal WaPOR Interception data'
    ...
    if level == 1:
        ...
        print('WaPOR Interception: Level 3 data only available in some areas'
              ' with specific data cube code below: ')

        catalog = WaPOR.API.getCatalog(version, level, True)
        for i, row in catalog.iterrows():
            if ('L3_I' in row['code']) & ('_D' in row['code']):
    ...
    try:
        ...
        raise Exception('WaPOR Interception ERROR: Cannot get cube info.'
    ...
    for index, row in df_avail.iterrows():
        print('WaPOR Interception: ----- {} -----'.format(index))
        ...
        print('WaPOR Interception: Downloaded file :', download_file)

        # Local raster file name
        filename = 'I_WAPOR.v%s_l%s-dekad-1_%s.tif' % (
            version, level,
            row['raster_id'])
        outfilename = os.path.join(Dir, filename)
        print('WaPOR Interception: Local      file :', outfilename)
        ...
        try:
            ...
            print("WaPOR Interception ERROR: %s - %s." % (err.filename, err.strerror))
...
def checkMemory(txt='', print_job=False):
    ...
    if print_job:
        print('WaPOR Interception: > Memory available      : {t} {v:.2f} MB'.format(
